...
import kr.co.ezenac.VO.AdminMemberMainVO;
import kr.co.ezenac.VO.AdminMemberVO;
import kr.co.ezenac.VO.AdminUpdateMemberVO;
...
public class AdminMemberDAO {
	...
	public List<AdminMemberMainVO> selectMemberAll() throws SQLException {
		...
		try {
			...
			while (rs.next()) {
				ammVO = new AdminMemberMainVO(rs.getString("MEM_NO"), rs.getString("MEM_ID"), rs.getString("MEM_NAME"),
	...
	public AdminMemberVO selectMember(String memId) throws SQLException {
		...
		try {
			...
			while (rs.next()) {
				admVO = new AdminMemberVO(rs.getString("mem_no"), rs.getString("mem_id"), rs.getString("mem_name"),
						rs.getString("mem_birth"), rs.getString("mem_gender"), rs.getString("mem_email"),
						rs.getString("mem_phone"), rs.getString("mem_regdate"));
			} // end while
		} finally {
			dc.dbClose(con, pstmt, null);
		} // end finally
	...
	public int updateMember(AdminUpdateMemberVO aumVO) throws SQLException {
		...
		try {
			con = dc.getConn();
			// String updateMember = "UPDATE MEMBER SET MEM_NAME = 'QWE', WHERE MEM_ID =
			// 'han' ";
